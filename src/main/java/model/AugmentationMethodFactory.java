...
import model.brightness.BrightnessMethodGPU;
...
public abstract class AugmentationMethodFactory {
    private static final boolean IS_CPU = false;
    ...
    public static BrightnessMethodThread createBrightnessMethodThread(float brightness, BufferedImage image, Vector<BufferedImage> storage) {
        return (IS_CPU) ? new BrightnessMethodCPU(brightness, image, storage) : new BrightnessMethodGPU(brightness, image, storage);
    }
}
